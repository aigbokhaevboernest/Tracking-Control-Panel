  ...
  async function doSave(values: FormValues, sendEmailFlag: boolean) {
    ...
    try {
      ...
      if (sendEmailFlag && values.receiver_email) {
        const ctx = {
          tracking_number: values.tracking_number, sender_name: values.sender_name,
          sender_country: values.sender_country, receiver_name: values.receiver_name,
          receiver_country: values.receiver_country, package_type: values.package_type,
          weight: values.weight, origin_label: values.origin_label,
          destination_label: values.destination_label, current_location: values.current_location,
          expected_delivery_date: values.expected_delivery_date, hold_amount: values.hold_amount,
        };
        const tpl = shipmentId
          ? buildStatusEmail(values.status ?? null, ctx) ?? buildCreatedEmail(ctx)
          : buildCreatedEmail(ctx);
        const result = await sendMail({
          email: values.receiver_email, subject: tpl.subject,
          first_name: values.receiver_name?.split(" ")[0] ?? "", message: tpl.message,
        });
